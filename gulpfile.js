...
gulp.task('replaceNames', function () {
	...
	return gulp.src(paths.toReplace.src)
		.pipe(stringReplace('Flexbox', config.theme.prefix))
		.pipe(stringReplace('FLEXBOX', config.theme.singleton))
		.pipe(stringReplace('flex-box', config.theme.textdomain))
		.pipe(stringReplace('Flex Box', config.theme.name))
		.pipe(stringReplace('WPGuide.Me', config.theme.author))
		.pipe(gulp.dest(function(file){
			return file.base;  // THE SAME DIR
		}));
});
